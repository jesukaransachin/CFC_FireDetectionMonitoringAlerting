...
public class IoTStarterApplication extends Application {
    ...
    private Integer fireTemp = 0;
    private Integer windSpeed = 0;
    private Double bioFuelWeight = 0.0;


    // Message log for log activity
    private final ArrayList<String> messageLog = new ArrayList<String>();
    GetDataService weatherService = RetrofitClientInstance.getRetrofitInstance().create(GetDataService.class);
    ...
    public Integer getFireTemp(){
        Integer temp = 0;
        Random ran = new Random();

        if(fireTemp == 0){
            temp = ran.nextInt(6) + 32;
        }else if(fireTemp == 1){
            temp = ran.nextInt(10) + 45;
        }else if(fireTemp ==2){
            temp = ran.nextInt(10) + 60;
        }
        return temp;
    }

    public Double getBioFuel(){
        bioFuelWeight = bioFuelWeight +0.01F;
        return bioFuelWeight;
    }
    ...
    public GetDataService getWeatherService(){
        if(getCurrentLocation()!= null) {
            Map<String, String> dataLocation = new HashMap<>();
            String lat = Double.toString(getCurrentLocation().getLatitude());
            String log = Double.toString(getCurrentLocation().getLongitude());

            Call<WeatherData> call = weatherService.getTemperature(lat,log);
            call.enqueue(new Callback<WeatherData>() {
                @Override
                public void onResponse(Call<WeatherData> call, Response<WeatherData> response) {
                    WeatherData data = response.body();
                    temperature = data.observation.temp;
                    windSpeed =data.observation.wspd;
                }

                @Override
                public void onFailure(Call<WeatherData> call, Throwable t) {
                    Log.d("Temperature","Failed to fetch temperature");
                }
            });
        }
    ...
    public void callAsynchronousWeatherTask() {
